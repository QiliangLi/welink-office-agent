...
- `server/` must bind to loopback only (non-loopback hosts are refused at startup), expose only `/api/v1`, reuse `scripts/lib/` services for every state change, never spawn `node scripts/agent.mjs` for writes, and never return runtime paths, CLI output, or `w3account`. Every POST passes through the persistent idempotency layer (`runtime/idempotency/`); server route schemas are the single source of the HTTP contract mirrored in `web-console/src/api/contracts.ts`.
