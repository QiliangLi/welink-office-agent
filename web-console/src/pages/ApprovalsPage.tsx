...
export function ApprovalsPage() {
  ...
  return (
    <div className="page approvals-page">
      ...
      {pending.length ? <div className="approvals-layout"><div className="approval-list">{pending.map((approval) => <ApprovalCard key={approval.id} approval={approval} onResolve={(status) => handleResolve(approval.id, status)} />)}</div><aside className="approval-tips"><div className="tips-mascot"><span><Lightbulb /></span><AgentMascot mood="approval" size="lg" /></div><h2>人工参与小贴士</h2><div><span><ShieldCheck /></span><p><strong>重要操作需确认</strong>涉及对外发送、费用或多人影响的动作会停下来等你。</p></div><div><span><Clock3 /></span><p><strong>及时处理更高效</strong>待处理项不会自动批准，任务会安全地停在当前步骤。</p></div><div><span><Pencil /></span><p><strong>不确定可修改</strong>先调整内容或补充信息，再让 Agent 继续执行。</p></div><Link to="/tasks" className="tips-link"><MessageSquareText />有疑问？查看相关任务 <HelpCircle /></Link></aside></div> : <section className="empty-approvals"><AgentMascot mood="empty" size="lg" /><h2>当前没有待处理事项</h2><p>Agent 会继续推进任务，有需要确认的动作会出现在这里。</p><Link className="button button-primary" to="/tasks">查看全部任务</Link></section>}
