    ...
    <div className="page tasks-page">
      ...
      <div className="tasks-layout">
        ...
        <aside className="tasks-rail">
          <section className="rail-card quick-filter"><header><h2>快速筛选</h2><AgentMascot mood="waiting" size="sm" /></header>{statusCounts.filter(({ count }) => count > 0).map(({ status: value, count }) => <button key={value} className={status === value ? "active" : ""} onClick={() => setStatus(value)}><span>{value === "completed" ? <Check /> : value === "waiting_approval" ? <UserRound /> : <SlidersHorizontal />}{STATUS_META[value].label}</span><b>{count}</b></button>)}<button className="clear-filter" onClick={clear}>清除筛选条件</button></section>
          ...
          <section className="rail-card new-task-promo"><div><h2>需要创建新任务？</h2><p>用一句话说明目标，Agent 会拆解计划并持续跟进。</p><Link className="button button-primary" to="/tasks/new"><Plus />下发新任务</Link></div><AgentMascot mood="approval" size="md" /></section>
        </aside>
      </div>
    </div>
  );
