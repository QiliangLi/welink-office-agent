import { ArrowLeft, ArrowRight, CalendarDays, Check, ChevronDown, FilePlus2, Flag, Lightbulb, Lock, Paperclip, Save, ShieldCheck, Sparkles, WandSparkles, X } from "lucide-react";
...
export function NewTaskPage() {
  ...
  const checks = useMemo(() => [
    ...
    { label: "截止时间", value: new Date(form.deadline).toLocaleString("zh-CN", { month: "2-digit", day: "2-digit", hour: "2-digit", minute: "2-digit", hour12: false }), pass: Boolean(form.deadline) },
  ...
  const submit = () => { const task = createTask(form); localStorage.removeItem(draftKey); setCreatedId(task.id); };

  if (createdId) {
    return <div className="page create-success"><div className="success-card"><AgentMascot mood="success" size="lg" /><span className="success-icon"><Check /></span><h1>任务已创建</h1><p>Agent 正在拆解执行计划。涉及外部发送或高风险操作时，会先回到“待我处理”等你确认。</p><code>{createdId}</code><div><Link className="button button-primary" to={`/tasks/${createdId}`}>查看任务 <ArrowRight /></Link><button className="button button-secondary" onClick={() => { setCreatedId(null); setForm(defaultForm); }}>继续创建</button></div></div></div>;
  }
      ...
      <div className="new-task-layout">
        <form className="task-form" onSubmit={(event) => { event.preventDefault(); if (!descriptionError) setConfirmOpen(true); }}>
          <section className="form-card prompt-card">
            ...
            <div className="attachment-row"><label className="button button-secondary"><Paperclip />添加附件<input type="file" className="sr-only" onChange={(event) => { const file = event.target.files?.[0]; if (file) setAttachments((current) => [...current, file.name]); }} /></label><span>支持文档、表格、图片，单文件不超过 20MB。</span></div>
            {attachments.length > 0 && <div className="attachment-chips">{attachments.map((name) => <span key={name}><FilePlus2 />{name}<button type="button" onClick={() => setAttachments((current) => current.filter((item) => item !== name))} aria-label={`移除 ${name}`}><X /></button></span>)}</div>}
          </section>
          ...
          <section className="form-card advanced-card">
            ...
            {advanced && <div className="advanced-grid">
              <fieldset><legend><Flag />优先级</legend><div className="segmented-control">{(["low", "normal", "high", "urgent"] as const).map((value) => <label key={value} className={form.priority === value ? "selected" : ""}><input type="radio" name="priority" value={value} checked={form.priority === value} onChange={() => update("priority", value)} />{{ low: "低", normal: "普通", high: "高", urgent: "紧急" }[value]}</label>)}</div></fieldset>
