...
- 本机 Console API（`server/`，默认 `127.0.0.1:4174`）：只读 DTO（任务、审批、跨任务动态 `GET /api/v1/activity`）、创建任务、暂停/继续/取消、审批决定、催办、可联系同事配置（`/api/v1/contacts`）、SSE 实时通知；
- 桌面 Web 控制台接入真实 API（`VITE_DATA_SOURCE=mock` 可切回演示数据）：总览、任务、待我处理、动态、产物、设置六个页面；
- 默认 dry-run。

尚未实现：附件上传（页面隐藏入口）、产物列表与下载（产物页显示能力说明，不展示虚构产物）、除联系人以外的配置写入（owner、群组、路由、dry-run 仍为只读）、多用户/远程访问。控制台不能直接调用 `welink-cli`，也不能读取 `runtime/` 文件。
...
`policies.json` 另有两个与外发相关的可调字段：`send_timeout_ms` 是单次 `welink-cli` 发送的超时（毫秒，缺省 60000）；超时的发送结果会记为"未知"，保持联系人沟通槽占用直到宿主核实并显式关闭会话。不要把它设得过小（建议不低于 10000），否则网络抖动会产生大量待核实动作。
...
页面默认读取真实 API；设置 `VITE_DATA_SOURCE=mock` 可切换到内置演示数据（用于离线开发与视觉验证）。顶栏会显示 Agent 健康状态、命令积压和 dry-run 标识；Agent 未运行时任务保持"待执行"，页面会如实显示等待原因，不会假装已开始联系同事。「动态」页跨任务查看已记录的事件与消息（`GET /api/v1/activity`，支持类型/任务/时间筛选和分页）；「设置」页展示当前用户（员工号脱敏）、运行模式和健康状态，并支持管理可联系同事（新增、编辑、移除及「允许主动联系」开关，直接写入本地 `config/contacts.json`，页面不会显示工号之外的账号信息）；「产物」页在能力接入前只显示说明，不出现虚构产物。
