...
Lifecycle rules for terminal tasks (review W-01/X-01): when a task is cancelled or completed, conversations whose outbound action is still `executing`/`unknown` stay active — handing the slot to the next task early would allow two overlapping private sends to the same contact, or a send whose delivery is unverified. The send's own completion path (executeSend) closes such a conversation only once the action settles to a KNOWN outcome (`succeeded`/`dry_run`/`failed`) and then promotes the next valid candidate; `unknown` results keep occupying the slot until the host recovery flow verifies the outcome and closes the conversation explicitly. A task that finished while its CLI call was running never has its subtask revived to `waiting_reply`.
